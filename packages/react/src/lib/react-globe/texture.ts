/* eslint-disable max-len */

// The land, as a picture 256 pixels across and 128 high with a bit to the pixel: white where
// there is land and black where there is sea, laid out the way a map is. It is read a dot at a
// time by the globe's fragment shader, which is why it is so small, and carried as a data URI so
// that a globe draws its land without reaching for anything across the network.
//
// Traced from https://de.wikipedia.org/wiki/Datei:World_map_blank_without_borders.svg
export const GLOBE_TEXTURE =
    "data:image/png;base64,";

/* eslint-enable max-len */
